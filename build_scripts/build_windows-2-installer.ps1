...
mkdir build_scripts\win_build -ea 0
...
Write-Output "   ---"
Write-Output "Copy chia executables to packages\gui\"
Write-Output "   ---"
Copy-Item "build_scripts\dist\daemon" -Destination ".\packages\gui\" -Recurse
...
Write-Output "   ---"
Copy-Item ".\dist\win-unpacked" -Destination "$env:GITHUB_WORKSPACE\Chia-win32-x64" -Recurse
mkdir "$env:GITHUB_WORKSPACE\release-builds\windows-installer" -ea 0
Copy-Item ".\dist\ChiaSetup-$packageVersion.exe" -Destination "$env:GITHUB_WORKSPACE\release-builds\windows-installer"
