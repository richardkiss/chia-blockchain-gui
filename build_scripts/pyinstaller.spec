...
# Find chia in site-packages
chia_spec = importlib.util.find_spec("chia")
if chia_spec:
    ROOT = pathlib.Path(chia_spec.origin).parent.parent
else:
    raise ImportError("chia package not found")

# Workspace root for external binaries
PROJECT_ROOT = pathlib.Path(os.environ.get("GITHUB_WORKSPACE", os.getcwd()))
...
if os.path.exists(f"{PROJECT_ROOT}/madmax/chia_plot"):
    binaries.extend([
        (
            f"{PROJECT_ROOT}/madmax/chia_plot",
            "madmax"
        )
    ])

if os.path.exists(f"{PROJECT_ROOT}/madmax/chia_plot_k34",):
    binaries.extend([
        (
            f"{PROJECT_ROOT}/madmax/chia_plot_k34",
            "madmax"
        )
    ])

if os.path.exists(f"{PROJECT_ROOT}/bladebit/bladebit"):
    binaries.extend([
        (
            f"{PROJECT_ROOT}/bladebit/bladebit",
            "bladebit"
        )
    ])

if os.path.exists(f"{PROJECT_ROOT}/bladebit/bladebit_cuda"):
    binaries.extend([
        (
            f"{PROJECT_ROOT}/bladebit/bladebit_cuda",
            "bladebit"
        )
    ])
...
if THIS_IS_WINDOWS:
    ...
    binaries = [
        ...
        (
            f"{PROJECT_ROOT}\\madmax\\chia_plot.exe",
            "madmax"
        ),
        (
            f"{PROJECT_ROOT}\\madmax\\chia_plot_k34.exe",
            "madmax"
        ),
        (
            f"{PROJECT_ROOT}\\bladebit\\bladebit.exe",
            "bladebit"
        ),
        (
            f"{PROJECT_ROOT}\\bladebit\\bladebit_cuda.exe",
            "bladebit"
        ),
    ]
